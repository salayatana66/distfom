/********************************************************************************************
 *  Test for Correctness of AdaGrad
 *  Constructs a ranking model and one observes the loss
 *  going down with small learning rates
 *
 ...
 *  val myS = AutoPasteFromScript("src/test/scala/AdaGradTest.scala", $intp)
********************************************************************************************/
...
import distfom.{ArrayBatcher, Example, Models, DistributedStackedDenseVectors => DSV,
  FomDistStackedDenseVec => FDSV, AdaptativeGradientDescent => AdaSGD}
// logger
...
object AdaGradSigmoidPairedRankingLoss {
 ...
 */
  ...
  @transient val chkPath = new Path("/tmp/AdaSGDTest")
  hfs.mkdirs(chkPath)
  sc.setCheckpointDir("/tmp/AdaSGDTest")
  ...
  // optimizer
  val AdaSGDMin = new AdaSGD[FDSV[Double], Double](
    defaultStepSize = .1,  l2reg = .5e-4,  l1reg = .5e-6,
    maxAge = 12,
    maxIter = 50 )

  val finalWeight = AdaSGDMin.minimize(f = sigLoss, init = fvecDoubleStart)
